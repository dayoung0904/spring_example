...
<script>
	$(document).ready(function(){
		/* // 1) jquesry submit 기능 이용하기(form 태그)
		$("form").on("submit", function(e){
			e.preventDefault(); // form 태그 동작 막음
			//alert("폼 태그 동작")
			...
			if(!email){
				alert("이메일을 입력하세요");
				return false; // submit 안함 form 태그 일때만 사용함
			}
		...
		$('#joinBtn').on('click', function(){
			...
			$.ajax({
				// request
				type:"POST" // type(key)은 대문자로 쓰면 안됨
				, url:"/lesson06/ex01/add-user"
				, data:{"name":name, "yyyymmdd":yyyymmdd, "email":email, "introduce":introduce} // JSON String 으로 보냄 "json key":val로 꺼내온 값
				
				// response
				// call back 함수
				, success:function(data) { // data: response 결과 응답, request의 data와는 아무 관계 없음
